...
function gatewayErrorStatus(code) {
  ...
  if (code === "SYNC_CODE_NOT_FOUND") return 404;
  if (code === "SYNC_DATA_TOO_LARGE") return 413;
  if (code === "SYNC_DATA_CORRUPT") return 500;
  if (code === "STATUS_TOO_LARGE") return 413;
  if (code === "CLASSROOM_DATA_CORRUPT") return 500;
  if (code === "GALLERY_DATA_CORRUPT") return 500;
  return 400;
}
  ...
  return http.createServer(async (req, res) => {
    ...
    try {
      if (url.pathname === "/api/gateway") {
        ...
        const result = await handleGatewayOperation(body, req);
