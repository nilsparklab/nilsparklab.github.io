...
(function () {
  ...
  window.NIL_API = Object.freeze({
    ...
    syncPush: (code, data) => request('/api/gateway', { method: 'POST', body: { operation: 'sync-push', code, data } }),
    syncPull: (code) => request('/api/gateway', { method: 'POST', body: { operation: 'sync-pull', code } }),
    classPush: (room, studentId, studentName, status) => request('/api/gateway', { method: 'POST', body: { operation: 'class-push', room, studentId, studentName, status } }),
    classPull: (room) => request('/api/gateway', { method: 'POST', body: { operation: 'class-pull', room } }),
    galleryPublish: (title, authorName, code) => request('/api/gateway', { method: 'POST', body: { operation: 'gallery-publish', title, authorName, code } }),
    galleryList: () => request('/api/gateway', { method: 'POST', body: { operation: 'gallery-list' } }),
